...
**C++20** + **CMake 3.20+** is the runtime. SQLite file store at. HTTP via cpp-httplib.
...
| Calibration | Logistic blend of those signals + a modest post-ChatGPT date prior; rank mix; bands `LIKELY_AI` / `LIKELY_HUMAN` / `UNCERTAIN` |
| Graph | Per-collection kNN on embedding cosine. The UI draws a circular cluster layout. |
...
| GraphSAGE | Gated. Mean-aggregation fallback. |
...
- **Document graph** — circular cluster layout (unlabeled dots, one heading per cluster). Hover a dot for the title. Edges are embedding cosine. Click a heading or cluster to zoom; analysis follows that visible set.
- **AI share over time** — yearly estimate binned by **last-modified**, not the year the prose was written. A later date can mean an edit. If the collection has no dates (common for many PDF uploads), the chart stays hidden.
